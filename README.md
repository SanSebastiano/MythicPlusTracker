...
### Dashboard
- Displays a card for every dungeon available in the current Mythic+ season.
- Each card shows:
  - **Overall stats** — all-time best key level, total runs, and successful completions.
  - **Weekly stats** — this week's best key level, runs, and successful completions.
- Key levels are color-coded by difficulty (Low → Very High).
